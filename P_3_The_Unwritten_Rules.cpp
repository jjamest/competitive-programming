...
int32_t main()
{
    ...
    vector<vector<int>> bus(n, vector<int>(2 * m));
    vector<int> order;

    vector<pair<int, int>> pos(n * 2 * m + 1);
    ...
    {
        auto [i, j] = pos[student];
        if (i != 0 && !occ[i - 1].empty())
        {
            cout << "no" << endl;
            return 0;
        }
        if (j >= m)
        {
            auto it = occ[i].lower_bound(m);
            if (it != occ[i].end() && *it < j)
            {
                cout << "no" << endl;
                return 0;
            }
        ...
        {
            auto it = occ[i].upper_bound(j);
            if (it != occ[i].end() && *it < m)
            {
                cout << "no" << endl;
                return 0;
            }
